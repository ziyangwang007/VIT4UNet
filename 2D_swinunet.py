...
#############################################################################################################################################
swin_unet = models.swin_unet_2d((128, 128, 3), filter_num_begin=64, n_labels=3, depth=4, stack_num_down=2, stack_num_up=2, 
...
model_checkpoint = keras.callbacks.ModelCheckpoint(filepath=os.path.join(weight_dir, '2dswin_unetcallback.h5'),monitor='val_loss', save_best_only=True)


swin_unet.compile(loss=hybrid_loss,
...
swin_unet.load_weights(os.path.join(weight_dir, '2dswin_unetcallback.h5'))
temp_out = swin_unet.predict([test_input,])
# y_pred = temp_out[-1]
y_pred = temp_out
