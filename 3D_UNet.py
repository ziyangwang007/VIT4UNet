...
# save results
pred_dir = os.path.join(_dir, '3DUnetpreds/')
...
def train():
    ...
    model_checkpoint = keras.callbacks.ModelCheckpoint(filepath=os.path.join(weight_dir, '3dunetcallback.h5'), monitor='val_loss', save_best_only=True)

    csv_logger = keras.callbacks.CSVLogger(os.path.join(log_dir,  '3dunet.txt'), separator=',', append=False)
    ...
    model.fit(imgs_train, imgs_masks_train, batch_size=16, epochs=150, verbose=1, shuffle=False, validation_data=(imgs_test,imgs_masks_test), callbacks=[model_checkpoint, csv_logger])    # [model_checkpoint, csv_logger] 
...
def predict():
    ...
    os.chdir(pred_dir)
    np.save('results_3dunet.npy',imgs_test_results)
