...
swin_unet3plus  = base.swin_unet_3plus_2d(input_size=(128, 128, 3), n_labels=3, filter_num_down= [64, 128, 256, 512],
    filter_num_skip=[32, 32, 32, 32], filter_num_aggregate = 256, 
    stack_num_down=stack_num_down, stack_num_up=stack_num_up, activation=activation, output_activation=output_activation,
    batch_norm=True, pool=True, unpool=True, deep_supervision=True, patch_size=(2, 2), num_heads=[4, 8, 8, 8], window_size=[4, 2, 2, 2], num_mlp=512, 
...
model_checkpoint = keras.callbacks.ModelCheckpoint(filepath=os.path.join(weight_dir, '2dunet3+callback.h5'),monitor='val_loss', save_best_only=True)


swin_unet3plus.compile(loss=[hybrid_loss, hybrid_loss, hybrid_loss, hybrid_loss, hybrid_loss],
...
swin_unet3plus.load_weights(os.path.join(weight_dir, '2dunet3+callback.h5'))
temp_out = swin_unet3plus.predict([test_input,])
y_pred = temp_out[-1]
