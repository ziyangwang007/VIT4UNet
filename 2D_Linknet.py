...
from Process_Data_for_2D_NoisyLabel_MultiRes import load_train_data, load_test_data, load_mask_train_original, preprocess_squeeze, load_mask_test
...
# save results
pred_dir = os.path.join(_dir, '2DLinknet/')
...
def train():
    ...
    model_checkpoint = keras.callbacks.ModelCheckpoint(filepath=os.path.join(weight_dir, '2dLinknetcallback.h5'), monitor='val_loss', save_best_only=True)

    csv_logger = keras.callbacks.CSVLogger(os.path.join(log_dir,  '2dLinknet.txt'), separator=',', append=False)
    ...
    model = sm.Linknet(input_shape=(256,256,1), encoder_weights=None)
    ...
    print('-'*30)
    os.chdir(weight_dir)
    model.save_weights('2dLinknet.h5')
...
def predict():
    ...
    model = get_unet()
    os.chdir(weight_dir)
    model.load_weights('2dLinknetcallback.h5')
    ...
    os.chdir(pred_dir)
    np.save('results_2dLinknet.npy',imgs_test_results)
