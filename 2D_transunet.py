...
# transunet = models.transunet_2d((128, 128, 3), filter_num=[64, 128, 256, 512], n_labels=3, stack_num_down=2, stack_num_up=2,
#                                 embed_dim=768, num_mlp=1024, num_heads=12, num_transformer=6,
#                                 activation='ReLU', mlp_activation='ReLU', output_activation='Softmax', 
#                                 batch_norm=True, pool=True, unpool='bilinear', name='transunet')

transunet = models.transunet_2d((256, 256, 1), filter_num=[32, 64, 128, 256], n_labels=1, stack_num_down=2, stack_num_up=2,
                                embed_dim=768, num_mlp=1024, num_heads=12, num_transformer=6,
                                activation='ReLU', mlp_activation='ReLU', output_activation='Softmax', 
                                batch_norm=True, pool=True, unpool='bilinear', name='transunet')
...
# model_checkpoint = keras.callbacks.ModelCheckpoint(filepath=os.path.join(weight_dir, '2dtransunetcallback.h5'),monitor='val_loss', save_best_only=True)


# transunet.compile(loss=hybrid_loss,
...
# transunet.load_weights(os.path.join(weight_dir, '2dtransunetcallback.h5'))
# temp_out = transunet.predict([test_input,])
# # y_pred = temp_out[-1]
# y_pred = temp_out
